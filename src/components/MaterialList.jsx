...
export default function MaterialList({ rows, field, label, emptyHint, dense = false }) {
...
export function CardListPanel({ selection, rows, onClear, hint }) {
  return (
        ...
        <>
          <div className="clp-head">
            <div className="clp-titles">
              ...
              <div className="clp-sub">{rows.length} material{rows.length === 1 ? '' : 's'}</div>
            </div>
          ...
          <div className="clp-body">
            <MaterialList rows={rows} field={selection.field} label={selection.label} dense
              emptyHint="No materials in this selection." />
          </div>
        </>
      ) : (
